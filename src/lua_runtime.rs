...
fn system(lua: &Lua, (args, stdin): (Table, Option<String>)) -> mlua::Result<mlua::String> {
...
fn hostname(lua: &Lua, _: ()) -> mlua::Result<mlua::String> {
...
pub(crate) fn create_porchetta_table(lua: &Lua) -> mlua::Result<Table> {
    ...
    porchetta_tbl.set("json", json::create_table(lua)?)?;
    Ok(porchetta_tbl)
}
