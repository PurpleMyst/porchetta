...
fn test_sync_fetches_and_fast_forwards_topic() {
    ...
    let manifest = b"return { topics = { test = { root = '.config/test', paths = {'config.txt'} } } }\n";
    ...
    let mut engine = PorchettaEngine::with_home(remote_store, remote_home.clone());
    ...
    let mut engine = PorchettaEngine::with_home(local_store, local_home.clone());
...
fn test_sync_pushes_topic_and_system_heads() {
    ...
    let remote_system_head = remote_store.get_topic_hostname_head("test", &hostname).unwrap();
    assert_eq!(
...
fn test_sync_aborts_on_diverged_topic() {
    ...
    assert!(err.contains("diverged"), "error should mention divergence: {err}");
}
